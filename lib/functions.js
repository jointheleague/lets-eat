...
getInfoWindow = function(marker) {
  ...
  if(marker.orgID) {
    if (marker.orgID.toUpperCase() === "SDFB") {
      ...
      currentImg='<a href="http://feedingamericasd.org/" target="_blank"><img src="/FSD_Logo_RGB.png" style="width: 70px;"/></a>';
    } else if(marker.orgID.toUpperCase() === "BOTH"){
      currentImg='<a href="http://www.sandiegofoodbank.org/" target="_blank"><img src="/SDFB.Color.Logo.PNG.png" style="width: 70px; margin-bottom: 5px;"/></a><br><a href="http://feedingamericasd.org/" target="_blank"><img src="/FSD_Logo_RGB.png" style="width: 70px; float: right;"/></a>';
    }
  }
...
console.log("name=" + marker.name + " and url=" + marker.url);
  return new google.maps.InfoWindow({

    content: // bad code, bad practice, bad person
    '<span class="infoWindowTitle">' + ((marker.url) && (marker.url !== "TBD") ? '<a href="' + marker.url + '" target="_blank" class="infowindow"> ' + marker.name + '</a>' : marker.name) + '</span>' +
