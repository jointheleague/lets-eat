...
getInfoWindow = function(marker) {
  ...
  return new google.maps.InfoWindow({
    content: currentImg +'<h2>' + ((marker.url) && (marker.url !== "TBD") ? "<a href='" + document.url + "' target='_blank'>" + marker.name + "</a>" : marker.name) + '</h2><br><small>' + marker.foods + '</small><br><small>' + marker.hours + '</small>'+urlInfo
  });
}
